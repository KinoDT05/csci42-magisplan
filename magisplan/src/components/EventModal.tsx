...
export default function EventModal({ isOpen, onClose, onRefresh, projectId, editingEvent, userRole }: EventModalProps) {
  const [form, setForm] = useState(initialFormState);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formError, setFormError] = useState("");
  const canEdit = userRole === "Moderator" || userRole === "Head";
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="">
      <div className="flex items-center justify-between border-b pb-4 mb-6 relative">
          <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold text-[var(--main)]">
                  {editingEvent ? (canEdit ? `Edit your ${form.eventKind}` : `${form.eventKind} Details`) : `Schedule a ${form.eventKind === 'meeting' ? 'meeting' : 'activity'}`}
              </h2>
          </div>
      ...
      <form onSubmit={handleModalSubmit} className="space-y-5 text-gray-800">
          <fieldset disabled={editingEvent !== null && !canEdit} className="space-y-5">
